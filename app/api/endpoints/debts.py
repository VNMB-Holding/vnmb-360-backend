...
def get_debts(
    upload_id: Optional[int] = Query(None, description="Filter debts by specific spreadsheet upload_id (defaults to latest upload if not provided)"),
    ...
    query = db.query(DebtControl).filter(DebtControl.upload_id == upload_id)
