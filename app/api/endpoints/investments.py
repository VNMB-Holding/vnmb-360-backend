...
def get_investments(
    upload_id: Optional[int] = Query(None, description="Filter investments by spreadsheet upload_id (defaults to latest upload)"),
    ...
    query = db.query(FinancialInvestment)
    if upload_id is not None:
        query = query.filter(FinancialInvestment.upload_id == upload_id)
